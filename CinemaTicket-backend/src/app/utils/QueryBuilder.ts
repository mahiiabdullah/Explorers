...
> {
  ...
  sort(): this {
    ...
    if (sortBy === "Idea" || sortBy === "comments") {
      this.query.orderBy = {
        [sortBy]: {
          _count: sortOrder,
        },
    ...
    console.log("orderby", this.query.orderBy);
  ...
  fields(
    fiedlParams?: string,
    extraSelect?: Record<string, boolean | Record<string, unknown>>
  ): this {
    const fieldsParam = fiedlParams;
    if (fieldsParam && typeof fieldsParam === "string") {
      ...
      if (extraSelect) {
        this.selectFields = { ...this.selectFields, ...extraSelect };
      }
